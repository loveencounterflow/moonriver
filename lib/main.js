(function() {
  ...
  Segment = class Segment {
    ...
    _transform_from_raw_transform(raw_transform) {
      ...
      //...................................................................................................
      if (this.is_sender) {
        if (this.modifiers.do_once_after) {
        ...
        this.call = (d, forward = true) => {
          ...
          this.transform(d);
          if (forward && (!this.modifiers.do_once_before) && (!this.modifiers.do_once_after)) {
            this.send(d);
          }
          return null;
        };
      ...
      this.send = (d) => {
        switch (d) {
          ...
          default:
            if (this.is_over) {
              throw new Error("^moonriver@3^ cannot send values after pipeline has terminated; " + `error occurred in segment idx ${this.idx} (${rpr(this._name_of_transform())})`);
            }
            this.output.push(d);
        }
        return null;
