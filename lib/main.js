(function() {
  'use strict';
  var $, Async_pipeline, Async_segment, GUY, Pipeline, Proto_segment, Reporting_collector, Segment, UTIL, alert, debug, def, echo, entries, get_async_types, get_base_types, get_sync_types, get_transform_types, help, hide, info, inspect, log, misfit, nameit, noop, plain, praise, rpr, stf, stf_prefix, transforms, urge, warn, whisper;
  ...
  Pipeline = (function() {
    ...
    class Pipeline {
      ...
      _protocol(journal_entry = null) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          segment = ref[i];
          d[segment.transform.name] = ' ';
        }
        if (journal_entry != null) {
          d[journal_entry.segment.transform.name] = rpr(journal_entry.d);
        }
        d.o = this._last_output !== misfit ? rpr(this._last_output) : rpr(this.output);
        this.journal.push(d);
        return null;
      }
      ...
      * walk() {
      ...
      //---------------------------------------------------------------------------------------------------------
      before_walk() {
        var i, j, len, len1, ref, ref1, segment;
      ...
      * _walk() {
        ...
        while (true) {
          ...
          if (this.protocol != null) {
            /* TAINT should use API */
            this._last_output = rpr(this.output);
          }
          ref = this.output;
          for (i = 0, len = ref.length; i < len; i++) {
            d = ref[i];
      ...
      static * walk_named_pipelines(named_pipelines) {
        var _, i, idx, j, k, len, len1, names, pipeline, pipelines, process, type, types, v;
  ...
  //###########################################################################################################
  module.exports = {Pipeline, Segment, Async_pipeline, Async_segment, Reporting_collector, transforms, Proto_segment, $, get_base_types, get_sync_types, get_async_types, get_transform_types, misfit};
