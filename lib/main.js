  ...
  this.Moonriver = (function() {
    ...
    class Moonriver extends this.Classmethods {
      ...
      constructor(raw_pipeline) {
        var i, idx, is_sender, is_source, last_idx, len, raw_transform, transform;
        super();
        this.first_input = [];
        this.last_output = [];
        this.pipeline = [];
        ...
        for (idx = i = 0, len = raw_pipeline.length; i < len; idx = ++i) {
          ...
          ({is_sender, is_source, transform} = this._get_transform(raw_transform));
          ((is_sender, is_source, transform, idx) => {
