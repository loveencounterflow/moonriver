(function() {
  'use strict';
  var Duct, GUY, Modified_transform, Moonriver, Segment, UTIL, add_length_prop, alert, debug, echo, help, info, inspect, isa, log, plain, pluck, praise, rpr, symbol, type_of, types, urge, validate, validate_optional, warn, whisper,
    splice = [].splice;

  //###########################################################################################################
  GUY = require('guy');

  ({alert, debug, help, info, plain, praise, urge, warn, whisper} = GUY.trm.get_loggers('MOONRIVER'));
  ...
  types.declare('drive_cfg', {
    tests: {
      ...
      "( x.laps is Infinity ) or @isa.cardinal x.laps": function(x) {
        return (x.laps === 2e308) || this.isa.cardinal(x.laps);
      }
    }
  });
        ...
        case 'function':
          switch ((arity = raw_transform.length)) {
            case 0:
              ...
              break;
            case 1:
              is_sender = modifiers.once_before_first || modifiers.once_after_last;
              transform = raw_transform;
              break;
            case 2:
              if (modifiers.once_before_first || modifiers.once_after_last) {
  ...
  Moonriver = (function() {
    ...
    class Moonriver {
      ...
      constructor(cfg) {
        ...
        GUY.props.def(this, 'is_over', {
          get: () => {
            return this.sources.every(function(source) {
      ...
      * [Symbol.iterator]() {
      ...
      //---------------------------------------------------------------------------------------------------------
      * walk(cfg) {
        var collection, collector, d, i, j, l, last_segment, len, len1, len2, len3, len4, len5, len6, len7, len8, m, n, q, r, ref, ref1, ref2, ref3, s, segment, t;
        if (this.has_run && !this.sources_are_repeatable) {
        ...
        try {
          ...
          for (l = 0, len2 = ref1.length; l < len2; l++) {
            ...
            this._drive(cfg);
            for (m = 0, len3 = collector.length; m < len3; m++) {
              d = collector[m];
              yield d;
            }
            collector.length = 0;
          ...
          this._drive(cfg);
          for (n = 0, len4 = collector.length; n < len4; n++) {
            d = collector[n];
            yield d;
          }
          collector.length = 0;
          ...
          for (q = 0, len5 = ref2.length; q < len5; q++) {
            ...
            segment.call(segment.modifiers.values.last, false);
            this./* forward */_drive(cfg);
            for (r = 0, len6 = collector.length; r < len6; r++) {
              d = collector[r];
              yield d;
            }
            collector.length = 0;
          ...
          for (s = 0, len7 = ref3.length; s < len7; s++) {
            ...
            segment.call(symbol.drop);
            this._drive(cfg);
            for (t = 0, len8 = collector.length; t < len8; t++) {
              d = collector[t];
              yield d;
            }
            collector.length = 0;
      ...
      //---------------------------------------------------------------------------------------------------------
      drive(cfg) {
        var R/* forward */, collection, i, j, l, len, len1, len2, len3, len4, m, n, ref, ref1, ref2, ref3, segment;
        if (this.has_run && !this.sources_are_repeatable) {
          throw new Error("^moonriver@12^ pipeline is not repeatable");
        }
        this.has_run = true;
      ...
      //---------------------------------------------------------------------------------------------------------
      _drive(cfg) {
        var do_exit, first_idx, i, idx, lap_count, last_idx, ref, ref1, ref2, segment;
        debug('^67-8^', cfg);
        ...
        while (true) {
          ...
          for (idx = i = ref = first_idx, ref1 = last_idx; (ref <= ref1 ? i <= ref1 : i >= ref1); idx = ref <= ref1 ? ++i : --i) {
            ...
            if (((ref2 = segment.modifiers.first) != null ? ref2 : false) && (segment.call_count === 0)) {
              segment.call(segment.modifiers.values.first, false);
            }
            //...................................................................................................
            /* forward */            debug('^67-13^');
            if (segment.is_source) {
            ...
            debug('^67-17^', segment.exit, segment.has_exited);
            if (segment.exit) {
              do_exit = true;
              break;
            }
      ...
      toString(current_idx) {
        ...
        for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
          ...
          if (segment.input !== prv_output) {
            parts.push(GUY.trm.green(rpr(segment.input)));
          }
          parts.push(idx === current_idx ? GUY.trm.reverse(GUY.trm.gold(segment._name_of_transform())) : GUY.trm.gold(segment._name_of_transform()));
