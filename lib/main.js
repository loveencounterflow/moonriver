(function() {
  'use strict';
  var $, Async_pipeline, Async_segment, GUY, Pipeline, Proto_segment, Reporting_collector, Segment, Transformer, UTIL, alert, debug, def, echo, entries, get_async_types, get_base_types, get_sync_types, get_transform_types, help, hide, info, inspect, log, misfit, nameit, noop, plain, praise, rpr, stf, stf_prefix, transforms, urge, warn, whisper;
  ...
  Segment = (function() {
    ...
    class Segment {
      ...
      constructor(cfg) {
        ...
        this.start = cfg.modifiers.start;
        this.stop = cfg.modifiers.stop;
      ...
      //---------------------------------------------------------------------------------------------------------
      _is_my_modifier(x) {
        return (x === this.start) || (x === this.first) || (x === this.last) || (x === this.stop);
      }
  ...
  Pipeline = (function() {
    ...
    class Pipeline {
      ...
      //---------------------------------------------------------------------------------------------------------
      push(...P) {
        var R, i, len, ref, segment, transform;
        R = P[0];
        /* TAINT move below line to types */
        if (R === Transformer || R.prototype instanceof Transformer) {
          R = new R();
        }
        if (R instanceof Transformer) {
          ...
          if (this.types.isa.sync_pipeline(R)) {
            ref = R.segments;
            for (i = 0, len = ref.length; i < len; i++) {
              segment = ref[i];
              this.push(segment);
      ...
      * walk() {
      ...
      //---------------------------------------------------------------------------------------------------------
      _before_walk() {
        var i, l, len, len1, len2, m, ref, ref1, ref2, segment;
        ...
        if (!this.has_started) {
          ...
          for (l = 0, len1 = ref1.length; l < len1; l++) {
            segment = ref1[l];
            if (segment.start !== misfit) {
              segment.send(segment.start);
            }
          }
        }
        ref2 = this.segments;
      ...
      * _prepare_after_walk() {
      ...
      //---------------------------------------------------------------------------------------------------------
      stop_run() {
        return [...this.stop_walk()];
      }

      run_and_stop() {
        return [...this.walk_and_stop()];
      }
  ...
  Async_pipeline = (function() {
    ...
    class Async_pipeline extends Pipeline {
      ...
      * _prepare_after_walk() {
      ...
      //---------------------------------------------------------------------------------------------------------
      async stop_run() {
        return [...((await this.stop_walk()))];
      }

      async run_and_stop() {
        return [...((await this.walk_and_stop()))];
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      async * walk_and_stop() {
        yield* (await this.walk());
        yield* (await this.stop_walk());
        return null;
      }
  ...
  //###########################################################################################################
  module.exports = {Pipeline, Segment, Async_pipeline, Async_segment, Reporting_collector, Transformer, transforms, Proto_segment, $, get_base_types, get_sync_types, get_async_types, get_transform_types, misfit};
